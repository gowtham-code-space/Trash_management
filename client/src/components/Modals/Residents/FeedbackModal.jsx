...
function FeedbackModal({ collector, onClose }) {
    ...
    function handleSubmit() {
        if (rating === 0) {
        ToastNotification(t('modals:feedback.toast_select_rating'), "warning");
        return;
        }
        setLoading(true);
        ...
        setTimeout(function() {
        setLoading(false);
        ToastNotification(t('modals:feedback.toast_submit_success'), "success");
        onClose();
        }, 1500);
    }
    ...
    return (
        ...
        <div className="relative bg-white border border-secondary rounded-veryLarge p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="text-center mb-8">
                ...
                <p className="text-sm text-secondaryDark mt-2 bg-secondary px-4 py-1 rounded-large inline-block">{t('modals:feedback.id_prefix')} {collector.id}</p>
            </div>

            <div className="w-full mb-8">
                <label className="text-sm font-bold text-secondaryDark uppercase tracking-wide mb-4 block text-center">
                    {t('modals:feedback.rate_experience')}
                </label>
            ...
            <div className="w-full mb-8">
                <label className="text-sm font-bold text-secondaryDark uppercase tracking-wide mb-3 block">
                    {t('modals:feedback.share_feedback')}
                </label>
                <textarea 
                    rows="5"
                    value={comment}
            ...
            <div className="grid grid-cols-2 gap-4 w-full">
                ...
                >
                    {loading ? t('modals:feedback.submitting') : (
                    <>
                        {t('modals:feedback.submit_feedback')}
                        <Check size={18} isPressed={false} isDarkTheme={true} />
                    </>
                    )}
                </button>
