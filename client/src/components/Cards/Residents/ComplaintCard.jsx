...
function ComplaintCard({ complaint, onClick }) {
  ...
  return (
    <div
      onClick={() => {ToastNotification(t('pages:cards.complaint.toast_showing_location'), 'info'), onClick(complaint)}}
      ...
      <div className="flex-1 flex flex-col gap-1.5 md:gap-2 min-w-0">
        <div className="flex items-start justify-between gap-2 md:gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs md:text-sm text-primary/60 truncate">
              {t('pages:cards.complaint.reported_by', { time: complaint.reportedTime, name: complaint.reportedBy })}
            </p>
          </div>
          <span className="px-2 md:px-3 py-1 bg-warning/20 text-primary text-[10px] md:text-xs font-medium rounded-small whitespace-nowrap ">
            {complaint.status === 'Resolved' ? t('common:resolved') : complaint.status === 'Pending' ? t('common:pending') : complaint.status}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm text-primaryLight px-1 md:px-2 py-1">
            ...
            <span className="hidden sm:inline">{t('pages:cards.complaint.show_location')}</span>
            <span className="sm:hidden">{t('pages:cards.complaint.location')}</span>
          </div>
          
          <span className={`text-[10px] md:text-xs md:font-medium font-bold ${complaint.votedByUser ? 'text-success' : 'text-error'}`}>
            {complaint.votedByUser ? t('pages:cards.complaint.voted') : t('pages:cards.complaint.not_voted')}
          </span>
        </div>
      </div>
    </div>
